...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_selector(".from-saathi .part", timeout=120_000)
        page.wait_for_timeout(600)
        refused = page.locator(".from-saathi.refusal").count() > 0
        parts = page.locator(".from-saathi:not(.refusal) .part")
        n = parts.count()
        ...
        if fb.count() == 0:
            ...
            check("FR-7 records the answer", "शुक्रिया" in page.inner_text("[data-fb]"))
